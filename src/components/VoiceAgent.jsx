          ...
          <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[85%] rounded-2xl px-4 py-3 ${
              ...
              <div className="flex items-start gap-2">
                {msg.role === 'assistant' && (
                  <Bot className="w-4 h-4 text-leaf-600 mt-0.5 shrink-0" />
                )}
                {msg.role === 'user' && (
                  <User className="w-4 h-4 text-leaf-100 mt-0.5 shrink-0" />
                )}
                <div>
                  <p className="text-sm whitespace-pre-line leading-relaxed">{msg.text}</p>
                  {msg.data && (
                    <div className="mt-2 p-2 bg-leaf-50 rounded-lg text-xs text-leaf-700">
                  ...
                  <p className={`text-[10px] mt-1 ${msg.role === 'user' ? 'text-leaf-200' : 'text-slate-400'}`}>
            ...
            <div className="bg-white text-slate-800 shadow-sm border border-slate-100 rounded-2xl rounded-bl-md px-4 py-3">
              <div className="flex items-center gap-2">
                ...
                <div className="flex gap-1">
                  <span className="w-2 h-2 bg-leaf-500 rounded-full animate-bounce" style={{ animationDelay: '0ms' }} />
                  <span className="w-2 h-2 bg-leaf-500 rounded-full animate-bounce" style={{ animationDelay: '150ms' }} />
                  <span className="w-2 h-2 bg-leaf-500 rounded-full animate-bounce" style={{ animationDelay: '300ms' }} />
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none">
          {quickActions.map((action) => (
            <button
              ...
              className="shrink-0 px-3 py-1.5 bg-leaf-50 text-leaf-700 text-xs font-semibold rounded-full hover:bg-leaf-100 transition-colors disabled:opacity-50"
            >
              {action.label}
            </button>
          ))}
      ...
      <div className="p-4 bg-white border-t border-slate-100 shrink-0">
        <form onSubmit={handleSubmit} className="flex items-center gap-2">
          ...
          <input
            ...
            disabled={isProcessing}
            className="flex-1 px-4 py-2.5 bg-slate-100 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-leaf-600 disabled:opacity-50"
          />

          <button
            type="submit"
            disabled={!inputText.trim() || isProcessing}
            className="w-11 h-11 bg-leaf-600 text-white rounded-xl flex items-center justify-center shrink-0 hover:bg-leaf-700 disabled:opacity-50 transition-colors"
          >
        ...
        {isSpeaking && (
          <div className="flex items-center gap-2 mt-2 px-1">
            <Volume2 className="w-3 h-3 text-leaf-600 animate-pulse" />
            <span className="text-[10px] text-leaf-600 font-medium">Speaking...</span>
            <button
